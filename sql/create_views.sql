...
disciplina.id inner join semestre on semestre.id = turma.id_semestre inner join curso on curso.id = turma.id_curso 
inner join formacao on formacao.id = curso.id_formacao inner join tipo_aula on tipo_aula.id = turma.id_tipo_aula inner join doc_indicado on
doc_indicado.id_turma = turma.id inner join docente on docente.id = doc_indicado.id_docente order by semestre.semestre DESC, disciplina.disciplina ASC, turma.turma ASC;
...
formacao.formacao, curso.curso as colegiado, docente.nome as docente_inteview_alocacao_interessado from turma inner join disciplina on turma.id_disciplina =
disciplina.id inner join semestre on semestre.id = turma.id_semestre inner join curso on curso.id = turma.id_curso 
inner join formacao on formacao.id = curso.id_formacao inner join tipo_aula on tipo_aula.id = turma.id_tipo_aula inner join doc_interessado on
doc_interessado.id_turma = turma.id inner join docente on docente.id = doc_interessado.id_docente order by semestre.semestre DESC, disciplina.disciplina ASC, turma.turma ASC;
...
create or replace view view_doc_indicado_computacao as select * from doc_indicado where doc_indicado.id_turma in (select view_turma_computacao.id from view_turma_computacao);
create or replace view view_doc_interessado_computacao as select * from doc_interessado where doc_interessado.id_turma in (select view_turma_computacao.id from view_turma_computacao);